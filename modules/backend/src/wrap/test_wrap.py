...
@pytest.fixture
def event():
    with open(os.path.join(os.path.dirname(__file__), 'event.json'), 'r') as json_file:
...
class Test_Entry(object):
    ...
    def test_wrap_response(self, mocker, patch_env_vars, event):
        context = MockContext("unwrap", "3.8")
        from wrap import lambda_handler
